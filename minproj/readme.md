...
- Обязательные поля: phone, password, name, surname, staff, is_active, date_joined
...
- После валидации:
  ...
  - При необходимости создание получателей (is_check=False)
...
- После валидации:
  ...
  - При необходимости создание получателей (is_check=True)
  - Повторная активация получателей при необходимости (is_active=True)
  - Оповещения событий "забыл пароль" от получателей
  ...
  - Создание/просмотр/редактирование/удаление(is_active=False) групп получателей
  ...
  - Просмотр/редактирование кроме полей staff, is_active и is_check/удаление(is_active=False) собственного профиля
  - Восстановление паролей для получателей
...
- После валидации:
  - Просмотр информации об активных учреждениях (is_active=True) и ведомствах
  - Просмотр информации об активных и валидных координаторах (is_check=True, is_active=True)
  - Оповещения о добавлении событий в маршрут получателя (is_check=True) по email (если есть) и в ЛК получателя
...
- ResetPasswords - сервис для смены пароля
---
...
---
### /password-reset/
- POST: Возвращает staff (роль юзера), user_id и key для сброса пароля по номеру телефона, если в БД есть пользователь с данным номером телефона.
Время жизни key 7 дней (Все пользователи)

### /password-reset/confirm/
- POST: Смена пароля по значению key (все пользователи)

### /password-reset/list/
- GET: Список запросов на смену пароля
...
---
### /upload-photo/
- POST: Добавление файла в локальное хранилище (все пользователи - необходимо для регистрации пользователей)
### /photo/{id}/
- GET: Получение URL файла
  - Все активные и валидные пользователи
---
...
- POST: Создание пользователя
  ...
  - Операторы (валидные и активные) - создание Получателя (is_check=False)
  - Координаторы (валидные и активные) - создание Получателя (is_check=True)
...
- PATCH: Редактирование профиля пользователя 
  ...
  - Операторы (валидные и активные) - редактирование собственного профиля, кроме staff, department_id, is_active и is_check (только администраторы)
...
- POST: Добавление получателей в группу
...
- GET: Получение списка ведомств 
  - Любой пользователь (необходимо для регистрации Оператора)
